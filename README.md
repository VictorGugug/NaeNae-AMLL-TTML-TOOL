...
- **Dual Format Support (TTML + Lyricsfile YAML)** : Full bidirectional editing, import, and export for Apple Music-like `.ttml` and Lyricsfile `.lyricsfile.yaml` (1.0 & 1.1 superset), supporting sections, multi-vocalist roles (`v1`-`v4`, `-bg`), syllable timing, ruby furigana, and namespaced `x_amll_tool` extensions.
- **Dedicated Lyricsfile Converter Dialog** : Accessible via Tools -> Lyricsfile Converter with drag-and-drop conversion, real-time validation, and instant preview.
- **Folder Projects & Workspace System** : Open, create, and save lyrics as project folders with companion audio, side-by-side TTML/YAML formats, and persistent workspace history.
- **Split Spectrogram Multi-Track & Reverse Zones** : Synchronized dual-track spectrogram for duet editing and reversible audio zones with virtual playback for timing backwards lines.
