...
import { previewModeTypeAtom, PreviewModeType } from "$/modules/settings/states/preview";
...
export const TimingOverview = memo(() => {
	...
	const handleLocateTiming = useCallback(() => {
		...
		let target = 0;
		const el = viewport.querySelector(`[data-line-index="${activeIndex}"]`) as HTMLElement | null;
		if (el) {
			target =
	...
	}, [currentTime, setSelectedLines, sortedLines, smoothScrollTo, cancelScrollAnimation]);
