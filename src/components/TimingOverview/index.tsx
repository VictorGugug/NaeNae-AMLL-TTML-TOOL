...
export const TimingOverview = memo(() => {
	...
	const handleRowClick = useCallback(
		...
		[setCurrentTime, setSelectedLines, audioPlaying],
	);
		...
		(word: LyricWord, line: LyricLine) => {
			...
			const targetTime =
				typeof word.startTime === "number" && word.startTime >= 0
					? word.startTime
					: line.startTime;
			const targetSec = targetTime / 1000;
			setCurrentTime(targetTime);
			setSelectedLines(new Set([line.id]));
			if (audioPlaying || audioEngine.musicPlaying) {
				void audioEngine.resumeOrSeekMusic(targetSec);
			} else {
				audioEngine.seekMusic(targetSec);
			}
		},
		[setCurrentTime, setSelectedLines, audioPlaying],
	);
	...
	const handleLocateTiming = useCallback(() => {
		...
		const activeIndex = sortedLines.findIndex(
			(l) => cur >= l.startTime && cur <= l.endTime,
		);
		...
		if (el) {
			...
			target =
				activeIndex * avgHeight - viewport.clientHeight / 2 + avgHeight / 2;
		}

		cancelScrollAnimation();
		smoothScrollTo(viewport, Math.max(0, target), 300);
	}, [
		currentTime,
		setSelectedLines,
		sortedLines,
	...
	useKeyBindingAtom(keyLocateActiveLineAtom, handleLocateTiming, [
		handleLocateTiming,
	]);

	useEffect(() => {
		if (!autoScroll) return;
		if (previewMode !== PreviewModeType.Timing) return;
		if (isPointerDownRef.current) return;
